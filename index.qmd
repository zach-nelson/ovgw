...
google-scholar: true
categories: [R, hydrology, monitoring wells, LADWP data, OVGA, time series]
---
...
Summary information about the processing, number of records dropped etc, and indicator well hydrographs for visual QA/QC are provided. The most recent OVGA depth to water import template instructions are included as an appendix. Before running the same code for the next year, update `year` in the file names for 1) [the water year on the input files and master database](#updatewy), 2) [output file name for the master database](#masterdb) and the 3) [ovga update file](#ovgaupdate). The [index.qmd](https://github.com/zach-nelson/ovgw/blob/main/index.qmd) of this repository contains the full code to process the data and produce this document.
...
-   [Completed 1-24-24 annual update](https://github.com/zach-nelson/ovgw/blob/main/output/testwellwy2023.csv)

-   [Completed 1-24-24 full database](https://github.com/zach-nelson/ovgw/blob/main/output/Monitoring_Wells_Master_2023.csv)
...
```{r lastyearjoinrp, echo = TRUE}
#| code-fold: true
#| code-summary: "filter last water year, join RP elevation column"

## Pull off last water year(annual update)

m1 <- testwells.combined %>% filter(date > "2022-09-30") %>% select(staid,date,dtw.rp)

# head(m1)
# 96,916 rows staid,date,dtw.rp

# join the rp elevations to the annual updates
# join latest rp elevation measurements provided by LA
...
# daily average dtw
dtwrpavg.rpelev <- dtwrp.rpelev %>% group_by(staid, date, rp_elev) %>% summarise(dtw.rp =  round(mean(dtw.rp),2))

dtwrpavg.rpelevBeta <- dtwrp.rpelev %>% group_by(staid, date, rp_elev) %>% summarise(dtw.rp =  round(mean(dtw.rp),2),
          dailyCount = n()
          ) %>% arrange(desc(dailyCount))

# dtwrpavg.rpelevBeta %>% select(staid, date, dtw.rp, dailyCount) %>% filter(dailyCount >25) %>% datatable()

# dtwrpavg.rpelevBeta %>% datatable()
#19865
...
[Completed 1-24-24](https://github.com/zach-nelson/ovgw/blob/main/output/ovga_uploads_mw_with_rpe_102022_092023_zn.csv)

[csv](https://raw.githubusercontent.com/zach-nelson/ovgw/main/output/ovga_uploads_mw_with_rpe_102022_092023_zn.csv)
...
```{r sumtables}
# head(testwells.combined %>% distinct(staid))
# 1363 unique staids
```

```{r sumtables2}
# testwells.combined %>% filter(year == 2023) %>% group_by(staid, year) %>% summarise(n = n()) %>% arrange(desc(n)) %>% datatable()
# staid measurements per year - 35,567 staid year rows
```
